...
vector< vector<float> > normalize(vector< vector <float> > grid) {
    ...
    for(i=0;i<grid.size();i++){
        ...
        for(j=0;j<row.size(); j++){
            p=row[j];
            single_row.push_back(p/total);
        }
    newGrid.push_back(single_row);
    }
        ...
        for(int j=0;j<width;j++){
            ...
            for(int dx=-1;dx<2;dx++){
                for(int dy=-1;dy<2;dy++){
                    ...
                    long newGrid_i=(i+dy+height) % height;
                    long newGrid_j=(j+dx+width) % width;
                    newGrid[newGrid_i][newGrid_j]+=mult*grid_val;
